...
def test_embedded(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_embedded_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@pytest.mark.macos
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_embedded_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_embedded_host(app: QemuApp, dut: QemuDut) -> None:
